...
function get_browsers($ua){
    ...
    if (preg_match('#MSIE ([a-zA-Z0-9.]+)#i', $ua, $matches)) {
		...
		$title = 'Maxthon '. $matches[2] . '国产马桶就是好用~';
		$icon = 'maxthon';
	}elseif (preg_match('#360([a-zA-Z0-9.]+)#i', $ua, $matches)) {
		$title = '360 Browser '. $matches[1] . '我敢肯定你电脑上全是360的东西!';
		$icon = '360se';
	}elseif (preg_match('#SE 2([a-zA-Z0-9.]+)#i', $ua, $matches)) {
		$title = 'SouGou Browser 2'.$matches[1] . ' 是不是输入法让你安这个浏览器的!';
		$icon = 'sogou';
	}elseif (preg_match('#UCWEB([a-zA-Z0-9.]+)#i', $ua, $matches)) {
		$title = 'UCWEB '. $matches[1] . ' 还真有用UC来这个网站的...';
		$icon = 'ucweb';
	}
	elseif(preg_match('#wp-android([a-zA-Z0-9.]+)#i', $ua, $matches))
	{
		$title = 'wordpress客户端'. $matches[1] . ' 又一个被WP毒害的孩纸...';
		$icon = 'wordpress';
	}
