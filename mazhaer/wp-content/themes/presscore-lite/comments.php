<div id="comments" class="comments-box">
    ...
    <?php if($comments) : //如果有评论 ?>
        <div id="commnents" class="commentsorping">
            <div class="commentpart active">评论<?php echo (' (' . (count($comments)-count($trackbacks)) . ')'); ?></div>
            <div class="pingpart">转载留言<?php echo (' (' . count($trackbacks) . ')');?></div>
        </div>
    ...
    <?php if(comments_open()) : ?>
        <div id="respond" class="respond" role="form">
            ...
            <?php if ( get_option('comment_registration') && !$user_ID ) : ?>
                <p>你必须 <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?redirect_to=<?php echo urlencode(get_permalink()); ?>">logged in</a> 才能发表评论.</p>
            <?php else : ?>
                <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
                    <?php if ( $user_ID ) : ?>
                        <p style="margin-bottom:10px">当前登录账号：<a href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a>&nbsp;|&nbsp;<a href="<?php echo wp_logout_url(get_permalink()); ?>" title="Log out of this account">退出 &raquo;</a></p>
                    <?php else : ?>
                        <div id="comment-author-info" <?php if ( !empty($comment_author) ) echo 'style="display:none"'; ?>>
                            <p>
                                <input type="text" placeholder="你的名字" required="" name="author" id="author" value="<?php echo $comment_author; ?>" size="14" tabindex="1" />
                                <label for="author">昵称</label>
                                <em>*</em></p>
                            <p>
                                <input type="text" placeholder="email@example.com" required="" name="email" id="email" value="<?php echo $comment_author_email; ?>"  pattern="^[A-Za-z0-9](([_\.\-]?[a-zA-Z0-9]+)*)@([A-Za-z0-9]+)(([\.\-]?[a-zA-Z0-9]+)*)\.([A-Za-z]{2,})$" size="25" tabindex="2" />
                    ...
                    <p class="comment-form-comment">
                        <textarea  placeholder="咳咳,说点啥吧..." required="" name="comment" id="comment" rows="10" tabindex="4" onkeydown="if(event.ctrlKey&&event.keyCode==13){document.getElementById('submit').click();return false};"></textarea>
                    </p>
                    ...
                    <input name="submit" type="submit" id="submit" class="commentsubmit" tabindex="5" value="发布（Ctrl + Enter）" />
